        ...
        try
            if typeof value is 'function' and value.prototype?
                ...
                if proto?
                    ...
                    switch baseName
                        when 'Slide'
                            classes.slides.push(value)
                        when 'Section'
                            classes.sections.push(value)
                        when 'Chapter'
                            classes.chapters.push(value)
                        when 'Presentation'
                            classes.presentations.push(value)
        catch error
            # 忽略错误
...
getProperties = (klass) ->
    ...
    for key, value of klass
        unless key in ['name', 'length', 'prototype', 'extend', 'include']
            props[key] = value
    props
...
autoGenerate = (outputPath = "outputs/auto-generated.html") ->
    ...
    console.log "📊 Found:"
    console.log "   - #{classes.slides.length} slides"
    console.log "   - #{classes.sections.length} sections"
    console.log "   - #{classes.chapters.length} chapters"
    console.log "   - #{classes.presentations.length} presentations"
    
    # 找到第一个 Presentation
    presentation = classes.presentations[0]
