...
HTML文件路径 = path.join __dirname, '../output/C01课程图表-一次成型版.html'
输出目录 = path.join __dirname, '../output'
图片目录 = path.join 输出目录, 'images-一次成型'
...
截图测试 = ->
  try
    ...
    for 容器, 索引 in 图表容器
      ...
      console.log "图表 #{索引 + 1}: #{尺寸.width} x #{尺寸.height}px"
    ...
    尺寸列表.forEach (图表) ->
      适合宽度 = 图表.宽度 <= 1123
      适合高度 = 图表.高度 <= 794
      console.log "  #{图表.标题}: 宽度#{if 适合宽度 then '✅' else '❌'} (#{图表.宽度}), 高度#{if 适合高度 then '✅' else '❌'} (#{图表.高度})"
