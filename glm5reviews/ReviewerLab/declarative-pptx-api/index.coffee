...
# 
# 可能的改进：
# 1. 根据属性数量动态调整高度，确保充分利用空间
# 2. 使用更智能的布局算法，根据内容长度调整高度
# 3. 考虑多行文本的情况，确保所有行都能显示
# 4. 使用SmartLayout.adjustFontSize()而不是@autoFontSize()，确保字体大小合适
# 
# 注意：
...
class QuoteSlide extends Slide
    @toPptx: (pptx) ->
        ...
        for key, value of properties
            ...
            slide.addText "\"#{value}\"",
                x: 1, y: y + 0.1, w: 8, h: 0.8
                fontSize: fontSize, italic: true, color: "666666"
            
            slide.addText "— #{key}",
                x: 1, y: y + 0.9, w: 8, h: 0.1
